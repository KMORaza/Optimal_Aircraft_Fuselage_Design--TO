...
FEMSolver::FEMSolver(Geometry& geometry, const Material& material)
    ...
    stiffness_matrix_.resize(num_dofs, num_dofs);
...
Eigen::MatrixXd FEMSolver::computeQuadElementStiffness(double t, double rho, double area) const {
    ...
    // Precomputed stiffness terms
    double diag_term = k * area / 6.0;
    double off_diag_term = k * area * nu / 12.0;

    for (int i = 0; i < 12; ++i) {
        Ke(i, i) = diag_term;
        if (i % 3 == 0 || i % 3 == 1) { // x, y directions
            Ke(i, (i + 3) % 12) = off_diag_term;
            Ke((i + 3) % 12, i) = off_diag_term;
        }
    }
...
void FEMSolver::assembleStiffnessMatrix(const std::vector<double>& density) {
    ...
    triplets.reserve(geometry_.getNumElements() * 12 * 12);

    for (int e = 0; e < geometry_.getNumElements(); ++e) {
        double t = geometry_.getSkinThickness();
        double rho = std::max(density[e], 0.1); // Minimum density
            ...
            for (int j = 0; j < 4; ++j) {
                for (int di = 0; di < 3; ++di) {
                    for (int dj = 0; dj < 3; ++dj) {
                        ...
                        triplets.emplace_back(global_i, global_j, Ke(i * 3 + di, j * 3 + dj));
                    }
                }
            }
        }
    ...
    for (int i = 0; i < num_dofs; ++i) {
        stiffness_matrix_.coeffRef(i, i) += 1e-2;
    }

    // Check for matrix singularity
    Eigen::VectorXd diag = stiffness_matrix_.diagonal();
    if (diag.minCoeff() < 1e-5) {
    ...
    for (int n = 0; n < geometry_.getNumNodes(); ++n) {
        ...
        if (std::abs(z) < 1e-6 || std::abs(z - geometry_.getLength()) < 1e-6) {
            for (int d = 0; d < dof_per_node_; ++d) {
                ...
                for (Eigen::SparseMatrix<double>::InnerIterator it(stiffness_matrix_, idx); it; ++it) {
                    it.valueRef() = 0.0;
                }
                stiffness_matrix_.coeffRef(idx, idx) = 1.0;
...
double FEMSolver::computeBucklingFactor() const {
    ...
    double k = 8.0; // More conservative buckling coefficient
    double nu = material_.getPoissonRatio();
    double D = E * std::pow(t, 3) / (12 * (1 - nu * nu));
    double critical_stress = k * M_PI * M_PI * D / (std::min(a, b) * t);
