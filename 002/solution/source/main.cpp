...
int main() {
    ...
    int num_frames = 20; // Increased for smaller panels
    int num_stringers = 40; // Increased for smaller panels
    ...
    TopologyOptimizer optimizer(fem_solver, 0.7); // Increased volume fraction
    optimizer.setMaterial(aluminum);
    optimizer.setConstraints(0.6, 0.01, min_thickness, aluminum.getYieldStrength());
    ...
    output.writeHeader({"Iteration", "Weight (kg)", "Compliance", "Max Stress (Pa)", 
                       ...
                       "Min Density", "Max Density", "Iteration Time (s)"});

    // Run optimization
    std::cout << "Starting topology optimization...\n";
    optimizer.optimize(50, 1e-2, output);
