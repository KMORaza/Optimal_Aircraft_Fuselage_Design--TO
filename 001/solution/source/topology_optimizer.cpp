...
void TopologyOptimizer::optimize(int max_iterations, double convergence_tol, Output& output) {
    ...
    for (int iter = 0; iter < max_iterations; ++iter) {
        ...
        double new_thickness = std::max(min_thickness_, avg_density * fem_solver_.getGeometry().getSkinThickness());
        fem_solver_.getGeometry().updateSkinThickness(new_thickness);

        // Compute sensitivities with stress penalty
        ...
        for (size_t e = 0; e < density_.size(); ++e) {
            double p = 2.5; // Reduced SIMP penalization for smoother convergence
            sensitivity_[e] = -p * std::pow(density_[e], p - 1) * fem_solver_.computeCompliance() * stress_penalty;
        }
        ...
        double condition_number = fem_solver_.computeConditionNumber();
        output.writeData({static_cast<double>(iter + 1), weight, fem_solver_.computeCompliance(), max_stress, 
                         ...
                         analytical_long, condition_number, min_density, max_density});
        ...
        if (!isValidDesign(violation)) {
            ...
            for (size_t e = 0; e < density_.size(); ++e) {
                density_[e] = std::min(1.0, std::max(0.1, density_[e] * 1.2)); // Stronger density increase
            }
            continue;
        }
